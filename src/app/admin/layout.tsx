...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const generateBreadcrumbs = () => {
    const pathSegments = pathname.split('/').filter(segment => segment && segment !== 'en' && segment !== 'admin');
    const breadcrumbs = [];
    let currentPath = '/en/admin';

    breadcrumbs.push(
      <BreadcrumbItem key="admin-home">
        <BreadcrumbLink asChild>
          <Link href="/en/admin/dashboard">Admin</Link>
        </BreadcrumbLink>
      </BreadcrumbItem>
    );
